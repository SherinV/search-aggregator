...
// from benchmark:
func insertQuery(tableName string, resources []map[string]interface{}, db *pgxpool.Pool, clusterName string) {
	...
	for _, resource := range resources {
		lastUID = strings.Replace(resource["uid"].(string), "local-cluster", clusterName, 1) //this will need to be replaced
		// var err error
		if SINGLE_TABLE {
			// edges := resource["edges"].(string)
			// edges = strings.ReplaceAll(edges, "local-cluster", clusterName)
			// _, err = db.Exec(context.Background(), lastUID, resource["data"], edges)
			edgesTo := resource["edgesTo"].(string)
			edgesTo = strings.ReplaceAll(edgesTo, "local-cluster", clusterName)
			edgesFrom := resource["edgesFrom"].(string)
			edgesFrom = strings.ReplaceAll(edgesFrom, "local-cluster", clusterName)

			var data map[string]interface{}
			bytes := []byte(resource["data"].(string))
			if err := json.Unmarshal(bytes, &data); err != nil {
				panic(err)
			}

			// fmt.Printf("Pushing to insert channnel... cluster %s. %s\n", clusterName, lastUID)
			resource := &dbclient.Record{TableName: tableName, UID: lastUID, Cluster: clusterName, Name: "TODO:Name here", Properties: data, EdgesTo: edgesTo, EdgesFrom: edgesFrom}
			dbclient.InsertChan <- resource

		} else {
			// _, err = statement.Exec(context.Background(), lastUID, resource["data"])

			resource := &dbclient.Record{UID: lastUID, Cluster: clusterName, Name: "TODO:Name here", Properties: resource["data"].(map[string]interface{})}
			dbclient.InsertChan <- resource
		}
		// if err != nil {
		// 	fmt.Println("Error inserting resource:", err, statement)
		// 	panic(err)
		// }
	}
